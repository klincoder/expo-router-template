...
const toastConfig = {
  ...
  info: ({ text1, text2, props, ...rest }) => (
    <BaseToast
      ...
      text2={text2}
      text2NumberOfLines={1}
      text1Style={tw`text-sm text-black font-regular`}
      text2Style={tw`text-base text-black font-medium`}
      style={tw`bg-accent border-accent h-20`}
      renderLeadingIcon={() => (
        <CustomIcon
          type="antDesign"
          name="infocirlceo"
          size={30}
          style={tw`text-black pl-5 self-center`}
        />
      )}
    />
  ), // close info
